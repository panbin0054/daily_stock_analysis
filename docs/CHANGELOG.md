...
- [修复] 持仓汇率刷新未覆盖外币账户本位币到总览币种的折算汇率，并在 Yahoo Finance 限流时缺少在线兜底，导致总览汇率状态持续显示过期。
- [改进] 持仓页止损接近预警展示触发账户、股票、亏损比例和预警线，便于定位具体风险项。
- [修复] 持仓页 A 股 ETF 缺少收盘价时改用轻量实时行情，并让止损预警忽略缺价持仓，避免缺价显示 0 并误触发告警。
- [修复] 回撤监控对含缺价持仓的每日快照使用账户上次有效估值延续，避免行情缺失日把持仓市值按 0 计入并放大最大回撤。
- [修复] 持仓页今日持仓实时价缓存增加短 TTL，避免首次进入页面后当天后续访问一直复用旧价格而不再刷新行情。
- [修复] 数据库单例初始化增加线程锁，避免服务重启后首波并发请求偶发拿到半初始化实例并返回 500。
- [修复] 持仓明细名称解析复用生成的股票索引，避免 `600160` 等未写入小型静态映射的股票显示为 `--`。
- [修复] 自选股分析入口按规范化股票代码自动去重，避免重复配置同一股票时并发生成内容不一致的重复报告。
- [修复] 大盘复盘历史详情改为直接展示复盘 Markdown 正文，避免继续渲染个股专用的策略点位、相关资讯和数据追溯空模块。
- [修复] Docker Web 服务默认使用 `--serve` 启动，使启用 `SCHEDULE_ENABLED` 时内置定时调度能实际运行。
- [改进] Web 定时任务保存调度开关或启动立即执行设置后自动重启服务，无需用户登录服务器或手动点击重启。
